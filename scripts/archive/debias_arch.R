...
for (j in 1:100) {
  ...
  for (i in 1:nboot) {
    ...
    # sigma2 <- ((n - sum(betanew != 0))^(-1)) * drop(crossprod(ynew - (xnew %*% betanew)))
    #
    # sd_norm_est <- numeric(ncol(xnew))
    # if (sum(betanew == 0) > 0) {
    #   sd_norm_est[betanew == 0] <- sqrt(sigma2 * (1 / diag(t(xnew[,betanew == 0]) %*% xnew[,betanew == 0])))
    # }
    # if (sum(betanew != 0) > 0) {
    #   sd_norm_est[betanew != 0] <- sqrt(sigma2 * diag(solve(t(xnew[,betanew != 0]) %*% xnew[,betanew != 0])))
    # }
  ...
  for (i in 1:ncol(zjs)) {
    ...
    curr_i <- which(order(norm_zjs, decreasing = TRUE)[1:100] == i)
