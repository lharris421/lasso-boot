...
for (i in 1:length(ns)) {
  ...
  for (j in 1:100) {
    ...
    #   lambdas[j, 1] <- lam
    #   widths[j,1] <- median(si_ci$upper - si_ci$lower) ## Only for variables included
    # }, error = function(e) {
    ...
    # widths[j,2] <- median(hdi_ci$upper - hdi_ci$lower)
    # }, error = function(e) {
    #   print(e); print("HDI failed")
    #   hdi_ci <- NULL
    # }
    # )
    ...
    lambdas[j, 3] <- lassoboot$lambda
    widths[j,3] <- median(lassoboot_ci$upper - lassoboot_ci$lower)
    ...
    # overall_coverage[j,] <- apply(coverage_df[,-c((ncol(coverage_df)-1) : ncol(coverage_df)),drop=FALSE], 2, mean, na.rm = TRUE)
  ...
  # mean_coverages <- apply(overall_coverage, 2, function(x) mean(x[!is.nan(x)]))
  # sd_coverages <- apply(overall_coverage, 2, function(x) sd(x[!is.nan(x)]))
  ...
  # mean_widths <- apply(widths, 2, median, na.rm = TRUE)
  # sd_widths <- apply(widths, 2, IQR, na.rm = TRUE)
  res_width[[i]] <- widths

}

save(res_time, res_coverage, res_lambda, res_width, ns, file = glue("./rds/method_comparison_laplace_100_{quantiles}_{method}_span1.rds"))
