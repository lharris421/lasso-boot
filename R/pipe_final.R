pipe <- function(X, y, fit, lambda, sigma,
  ...
  if (fit$family == "gaussian"){
    ...
    ## For elastic net??
    if (alpha < 1) {
      stop("alpha less than 1 not implimented for non-gaussian outcomes.")
      ## No equivalent trick that I know of...
      # yy <- c(yy, rep(0, p))
      # XX <- rbind(XX, sqrt(n*(1 - alpha)*lambda)*diag(p))
      # XX <- ncvreg::std(XX)
      # XX <- XX * sqrt(n / (n + p))
  ...
  if (fit$family == "gaussian") weights <- NULL

  if (posterior) {

    if (family == "gaussian") {
      ci <- ci_full_cond_pipe(beta_PIPE, lambda = lambda, se = sigma_PIPE,
...
ci_full_cond_pipe <- function(z, lambda, se, alpha, penalty = "lasso", family = "gaussian", weights = NULL) {
...
threshold_c <- function(bound, lambda, gamma, family = "gaussian", weights = NULL, penalty = "MCP") {
  ...
  if (penalty == "MCP") {
    ...
    z_j[cond] <- (gamma / (gamma - 1)) * soft_threshold_pipe(z_j[cond], lambda)
    ...
    out <- z_j
    # Region 1
    out[cond1] <- soft_threshold_pipe(z_j[cond1], lambda)

    # Region 2
    lambda_alt <- (gamma * lambda) / (gamma - 1)
    out[cond2] <- ((gamma - 1) / (gamma - 2)) * soft_threshold_pipe(z_j[cond2], lambda_alt)
...
soft_threshold_pipe <- function(z_j, lambda) {
