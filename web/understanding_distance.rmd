...
rlr <- function(dist1, dist2) {
  move <- sort(abs(dist1)) - sort(abs(dist2))
  move_left <- mean(move[move < 0])
  move_right <- mean(move[move > 0])
  exact <- mean(move == 0)
  if (is.nan(move_left) | is.nan(move_right)) {
    if (is.nan(move_left) & is.nan(move_right)) {
      return(0)
    } else if (is.nan(move_left)) {
      return(1)
    } else if (is.nan(move_right)) {
      return(-1)
    }
  }
  total_move <- mean(abs(move))
  return(((move_right - move_left) / total_move)^(exact))
}
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    observeEvent(input$go, {
      ...
      output$rlr <- renderText({
        paste0("Right-left Ratio: ", rlr(v1, v2), "\n")
      })
        }
      )
    })
