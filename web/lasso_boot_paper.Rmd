...
obs_lw <- pnorm(0, z + lambda*alpha, se, log.p = TRUE)
obs_up <- pnorm(0, z - lambda*alpha, se, lower.tail = FALSE, log.p = TRUE)

## Find the log density at zero for each tail (This needs to be finite)
dens0_lw <- dnorm(0, z + lambda*alpha, se, log = TRUE)
dens0_up <- dnorm(0, z - lambda*alpha, se, log = TRUE)
...
lower <- ifelse(
  ...
  qnorm(log(lower_p) + obs_lw - frac_lw_log, z + lambda*alpha, se, log.p = TRUE),
  qnorm(log(upper_p) + obs_up - frac_up_log, z - lambda*alpha, se, lower.tail = FALSE, log.p = TRUE)
)
upper <- ifelse(
  frac_lw_log >= log(upper_p),
  qnorm(log(upper_p) + obs_lw - frac_lw_log, z + lambda*alpha, se, log.p = TRUE),
  qnorm(log(lower_p) + obs_up - frac_up_log, z - lambda*alpha, se, lower.tail = FALSE, log.p = TRUE)
)
...
```{r}
ridge_credible_interval <- function(X, y, lambda, variance_estimate) {
  # Ensure that X is a matrix
  X <- as.matrix(X)
  
  # Compute the posterior mean and covariance
  sigma_squared <- variance_estimate
  XtX <- t(X) %*% X
  XtX_lambdaI <- XtX + lambda * diag(ncol(X))
  posterior_mean <- solve(XtX_lambdaI, t(X) %*% y)
  posterior_cov <- sigma_squared * solve(XtX_lambdaI)
  
  # Compute the standard deviations of the coefficients
  std_devs <- sqrt(diag(posterior_cov))
  
  # Compute the 80% credible interval
  z_score <- qnorm(0.9) # 80% interval, so 10% on each tail
  lower_bound <- posterior_mean - z_score * std_devs
  upper_bound <- posterior_mean + z_score * std_devs
...
ridge_credible_interval(xnew, ynew, lambda, sigma2)
```
